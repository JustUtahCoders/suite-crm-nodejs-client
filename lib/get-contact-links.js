...
(async () => {
  for (let page = 0; page < numContactPages; page++) {
    ...
    fs.writeFileSync(`./artifacts/contacts-${page * 20}-${page * 20 + 20}.json`, JSON.stringify(contacts, null, 2), "utf-8")
  }
})()
...
async function getLinks(contact) {
  ...
  const response = await fetch(`${process.env.SCRM_BASE_URL}/index.php?module=Contacts&offset=15&stamp=1593470833071698600&return_module=Contacts&action=DetailView&record=${contact.id}&ajax_load=1&loadLanguageJS=1`, {
    headers: {
  ...
  const documents = [], invoices = [], cases = []

  dom.window.document.querySelectorAll('#subpanel_documents tr').forEach((tr, i) => {
    const downloadAnchor = tr.querySelector('td a')
    if (downloadAnchor) {
      const downloadUrl = downloadAnchor.getAttribute('href')
      const filename = tr.querySelector('td:nth-child(3) a').textContent.trim()
      const id = new URLSearchParams(downloadUrl.slice(downloadUrl.indexOf('?'))).get('id')
      documents.push({downloadUrl, filename, id})
    }
  })

  dom.window.document.querySelectorAll('#subpanel_contact_aos_invoices tr').forEach((tr, i) => {
    ...
    if (viewInvoiceAnchor) {
      ...
      const action = new URLSearchParams(url.slice(url.indexOf('?'))).get('action')
      const id = new URLSearchParams(action).get('record')
      invoices.push({id})
    }
  })

  dom.window.document.querySelectorAll('#list_subpanel_cases tr').forEach((tr, i) => {
