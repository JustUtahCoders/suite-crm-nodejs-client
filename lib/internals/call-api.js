...
`.trimLeft();
  ...
  const response = await fetch(
    `${process.env.SCRM_BASE_URL}/service/v4_1/rest.php`,
    {
      method: "POST",
      body,
      headers: {
        "Content-Type":
          "multipart/form-data; boundary=---------------------------4362456108880421411137113181",
      },
    }
  );
  ...
  if (
    ...
    throw Error(
      `Invalid response calling ${method}. API returned status ${
        response.status
      } ${response.statusText} with content type ${response.headers.get(
        "content-type"
      )}`
    );
  }
...
export async function getSession() {
  if (!session) {
    const json = await callSuiteCrmApi(
      "login",
      {
        auth_data: {
...
export async function callCUDatabaseApi(url, opts = {}) {
  if (
    !process.env.CU_DATABASE_USERNAME ||
    !process.env.CU_DATABASE_PASSWORD ||
    !process.env.CU_DATABASE_URL
  ) {
    throw Error(
      `CU_DATABASE_USERNAME, CU_DATABASE_PASSWORD, and CU_DATABASE_URL env vars are required`
    );
  }
  ...
  const authToken = btoa(
    process.env.CU_DATABASE_USERNAME + ":" + process.env.CU_DATABASE_PASSWORD
  );
