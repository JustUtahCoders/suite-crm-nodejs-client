...
export async function insertClient(contact) {
  ...
  let phone =
    getValue("phone_home") ||
    getValue("phone_mobile") ||
    getValue("phone_work") ||
    getValue("phone_other") ||
    getValue("phone_fax") ||
    getValue("assistant_phone");
  phone = phone && phone.replace(/[\-a-zA-z\s]+/g, "").trim();
  const street =
    getValue("primary_address_street") +
    getValue("primary_address_street_2", "") +
    getValue("primary_address_street_3", "");
  const state =
    stateNameToAbbreviation[getValue("primary_address_state")] || null;

  const newClient = {
    dateOfIntake: dayjs(getValue("date_entered", currentDay)).format(
      "YYYY-MM-DD"
    ),
    firstName,
    lastName,
